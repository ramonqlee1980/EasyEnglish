...
#define kTimeoutForHTTPRequest 60.0f
...
-(void)saveDownloadedResources:(FileModel*)fileModel
{
    ...
    desFilePath = [desFilePath stringByAppendingPathComponent:fileModel.fileName];
    [CommonHelper copyFile:fileName toFile:desFilePath];
    
    [fileManager removeItemAtPath:fileName error:nil];
    
    if(fileModel.notificationName && fileModel.notificationName.length)
    {
        [[NSNotificationCenter defaultCenter]postNotificationName:fileModel.notificationName object:desFilePath];
    }
}
@end
